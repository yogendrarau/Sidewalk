...
| 2026-08-15 | Shopify legacy Admin tokens | Shopify's Dev Dashboard replaced legacy custom apps — a static `SHOPIFY_ADMIN_TOKEN` can no longer be minted for new apps | Ported teammate Yogi's OAuth install flow (NYCHackathon `sidewalk-version-b`, `functions/shopify_oauth`): `GET /shopify/oauth?shop=…` → authorize → HMAC-verified callback → offline token stored in `IntegrationCredential` (system-role only, never sent to a browser). Commerce functions read `shopifyCreds()` — env token wins, OAuth credential otherwise, simulated tier without either |
